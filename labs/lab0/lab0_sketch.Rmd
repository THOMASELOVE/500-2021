...
I will start with a quick summary to be sure things are imported properly in the `lab0` data set...
...
To store the linear predictions (i.e. log odds of the estimated probabilities,) and the estimated probabilities themselves as part of the lab0 data file, I'll use the following commands:
...
by(lab0$prob, lab0$treatment, describe)
```
...
```{r plot 3}
ggplot(lab0, aes(x = treatment, y = prob)) +
...
```{r plot 8}
ggplot(lab0, aes(x=linpred, fill=treatment)) + 
...
```{r plot 9}
ggplot(lab0, aes(x=linpred, color=treatment)) + 
  geom_density() +
  theme_bw()
```
...
outhist <- histbackback(split(lab0$prob, lab0$treatment), probability=TRUE,
                        xlim = c(-5,5),
...
`Call:  glm(formula = treatment == "Treated" ~ cov1 + cov2 + cov3 + cov4 + 
    cov5, family = binomial(), data = lab0)`
...
library(Epi)
par(mfrow=c(2,1))
ROC(form=(lab0$treatment=="Treated" ~ lab0$cov1 +
            lab0$cov2 + lab0$cov3 + lab0$cov4 + lab0$cov5))
```
